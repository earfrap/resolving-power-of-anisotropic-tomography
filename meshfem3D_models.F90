...
! ----------------------------------- delete this line by RAPPISI
!        call model_premsj_broadcast()
! -----------------------------------

  ! reads in 3D mantle models
  call meshfem3D_mantle_broadcast()
!print *,'ok call meshfem3D_mantle_broadcast'

  ! reads in crustal model
  call meshfem3D_crust_broadcast()
!print *, 'ok call meshfem_crust_broadcast'
 ...
 !print *,'ok subrountine meshfem3D_model_broadcast'
...
!---
  ...
  select case (REFERENCE_1D_MODEL)
    ...
    case (REFERENCE_MODEL_PREMSJ)   ! NEW LINE BY RAPPISI
      call model_premsj_broadcast(CRUSTAL)  ! NEW LINE BY RAPPISI
...
!---
 ...
 !print *, 'ok subroutine  meshfem_mantle_broadcast'
  ...
  select case (REFERENCE_1D_MODEL)
    ...
    case (REFERENCE_MODEL_PREM)
      ...
      if (TRANSVERSE_ISOTROPY) then
        ...
        vsh = vs
        eta_aniso = 1.d0
       ! c11 = ((rho*vp*vp)/(10.00**9))/scale_GPa
       ! c12 = ((rho*(vp*vp-2.d0*vs*vs))/(10.00**9))/scale_GPa
       ! c13 = c12
       ! c22 = c11
       ! c23 = c12
       ! c33 = c11
       ! c44 = ((rho*vs*vs)/(10.00**9))/scale_GPa
       ! c55 = c44
       ! c66 = c44
...
!!NEW LINES BY RAPPISI
    case (REFERENCE_MODEL_PREMSJ)
      ! PREMSJ - pure isotropic model, used in 1D model mode only
      call model_premsj(r_prem,rho,vp,vs,Qkappa,Qmu,iregion_code)
      vpv = vp
      vph = vp
      vsv = vs
      vsh = vs
...
!
  ...
  use meshfem3D_models_par
 use constants, only: R_EARTH,RHOAV,GRAV,PI
 use shared_parameters, only: NCHUNKS,NEX_XI,NEX_ETA,ANGULAR_WIDTH_XI_IN_DEGREES, &
                              ANGULAR_WIDTH_ETA_IN_DEGREES,CENTER_LATITUDE_IN_DEGREES, &
  ...
  ! local parameters
  double precision :: scaleval,scale_GPa,r_used,r_dummy,theta,phi,min_lon,max_lon,min_colat,max_colat,lon_step,colat_step
  double precision :: dvp,dvs,drho,vp,vs,moho,sediment,PwaveMod,SwaveMod,Nparam,Lparam,E_Chi
...
!---
  ...
  if (iregion_code == IREGION_CRUST_MANTLE) then
    ...
    if (MODEL_3D_MANTLE_PERTUBATIONS .and. r_prem > RCMB/R_EARTH .and. .not. suppress_mantle_extension) then
      ...
      select case (THREE_D_MODEL)
        ...
        if (r_prem>RCMB/R_EARTH .and. r_prem<RMOHO/R_EARTH)  then !!ELODIE 
  ...
  if (ANISOTROPIC_3D_MANTLE .and. iregion_code == IREGION_CRUST_MANTLE) then
    ...
    if (THREE_D_MODEL == THREE_D_MODEL_ANISO_MANTLE .or. THREE_D_MODEL == THREE_D_MODEL_ANISO_MANTLE_DREX) then
      ...
      ! c11,.. already set in model_aniso_mantle() e model_aniso_mantle_drex routine
      ! nothing to do left
      continue
    else
...
! -------------------------- Delete this line by Rappisi
   !  case (REFERENCE_MODEL_PREMSJ)
        ! 1D Ref changes Qmu at moho depth of 24.4km
        ! we take the crustal value and assign it to points only inside actual
        ! crust,
        ! otherwise the mantle values is taken
        ! makes sense especially for points below thin oceanic and thick
        ! continental crust
       ! if (CRUSTAL) then
          ! takes crustal Q value only if point is in actual crust
        !  if (r_prem > (ONE-moho) .or. elem_in_crust) then
...
! overwrites values with updated model values (from iteration step) here, given at all GLL points
  ...
  if (iregion_code /= IREGION_CRUST_MANTLE) return
   ...
   print *, 'call model gll_impose_vale'
